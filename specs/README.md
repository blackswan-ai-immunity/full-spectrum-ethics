...
`GovernanceEvent` is the local action record. `RiskAlert` interprets risk. `AuditTrace` records the processing and review path. `CrossEnterpriseAuditRecord` is a higher-level envelope for multi-party review.
